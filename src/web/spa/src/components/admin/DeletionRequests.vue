<template>
  <div class="mx-auto max-w-4.5xl">
    <h1 class="text-2xl font-medium">Pending account deletion requests</h1>
    <h4 class="pb-8">
      ...
      >
        <div class="my-2 flex items-center space-x-4">
          ...
          <div>
            ...
            <h4 class="text-[13px] leading-4 text-neutral-500">
  ...
  >
    ...
    >
      <div class="w-full space-y-5 px-6 pt-8 pb-5">
        <div class="text-base">
          Please explain why you want to decline this user's deletion request.
          <br />
          <span class="text-sm text-gray-400"
            >(E.g. "You have unpaid debt.")
          </span>
        </div>
        <textarea
          required
...
const [data, error] = await api.users.getDeletionRequests()
if (!error) requests.value = data

const acceptRequest = async request => {
  const [, error] = await api.users.updateDeletionRequest(request.user.email)
  if (!error) request.request.approved = true
}

const confirmDeclineRequest = async () => {
  const [, error] = await api.users.updateDeletionRequest(
