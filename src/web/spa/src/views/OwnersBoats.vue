<template>
 <div class="justify-between">
    ...
    <br>
    <div class="mx-auto max-w-5xl bg-white overflow-visible sm:rounded-md">
...
const fetchResults = async () => {
    const [data, error] = await api.business.searchBoats(
        {
            ...route.query
        },
        route.params.id
    ...
    if (!error) {
        data.forEach((b) => {
            b.values = [
                { name: 'Calendar', value: `/boat-profile/${b.id}/calendar`, icon: shallowRef(CalendarIcon)},
    ...
    if (value == "delete") {
        console.log("Deleted")
          ...
          router.push({
                ...
                params: { type: "boats", action: 'update' },
                query: { id: value[1]}
            })
    } else {
        router.push(value)
