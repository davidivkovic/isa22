  ...
  <div class="flex flex-col-reverse xl:mx-28 2xl:mx-56">
    ...
    >
      <h1 class="text-2xl font-semibold text-gray-900">
        <time :datetime="calendar.currentDate.toISOString().split('T')[0]">{{
          format(
            calendar.currentDate,
            calendarName == 'year' ? 'yyyy' : 'LLLL yyyy'
          )
        }}</time>
        <div class="-mb-2 text-base">{{ businessName }}</div>
      </h1>
...
import WeekCalendar from '@/components/business/calendar/WeekCalendar.vue'
import YearCalendar from '@/components/business/calendar/YearCalendar.vue'
import api from '@/api/api'

const businessTypes = {
  'adventure-calendar': 'adventures',
  'boat-calendar': 'boats',
  'cabin-calendar': 'cabins'
}
const route = useRoute()

const businessId = route.params.id
const businessType = businessTypes[route.name]
const [businessName] = await api.business.getName(businessId, businessType)
