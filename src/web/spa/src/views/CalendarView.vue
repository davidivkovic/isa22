<template>
  <div
    class="lg:flex lg:h-full lg:flex-col xl:px-20 xl:pb-10 2xl:px-56 2xl:pb-20"
  >
    ...
    >
      <h1 class="text-lg font-semibold text-gray-900">
        <time datetime="2022-01">{{ format(currentDate, 'LLLL yyyy') }}</time>
      </h1>
      <div class="flex items-center">
        ...
        <div class="hidden md:ml-4 md:flex md:items-center">
          ...
          <button
            ...
            type="button"
            class="ml-6 rounded-md border border-transparent bg-emerald-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-emerald-700"
          >
            Add event
          </button>
        </div>
              ...
              <li v-for="event in day.events.slice(0, 2)" :key="event.id">
                <a :href="event.href" class="group flex">
                  <p
                    ...
                    class="flex-auto cursor-pointer truncate font-medium text-gray-900 group-hover:text-emerald-600"
                  >
                    {{ event.name }}
                  </p>
                  <time
...
import {
  ...
  ChevronRightIcon,
  ClockIcon,
  DotsIcon as DotsHorizontalIcon
} from 'vue-tabler-icons'
import { Menu, MenuButton, MenuItem, MenuItems } from '@headlessui/vue'
import CreateEventModal from '@/components/business/view/CreateEventModal.vue'
...
const transformEvent = (datetime, event) => ({
  id: event.id,
  time: format(sub(parseISO(event.start), { hours: 2 }), 'h a'),
  name: event.name,
  datetime
})

const addEvent = event => {
  const datetime = event.start.split('T')[0]
  days.value[datetime].events.push(transformEvent(datetime, event))
}
...
const previousMonth = () => {
  currentDate.value.addDays(-currentDate.value.getDate() - 1)
  renderCalendar()
}
...
const renderCalendar = async () => {
  ...
  if (!error) {
    data.forEach(e => {
      const datetime = e.start.split('T')[0]
      days.value[datetime].events.push(transformEvent(datetime, e))
    })
  }
...
// ]
const selectedDay = Object.values(days.value).find(day => day.isSelected)
</script>
